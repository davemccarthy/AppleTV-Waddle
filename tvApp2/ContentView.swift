...
//  tvApp2
//
//  Created by David McCarthy on 18/10/2022.
//

import SwiftUI

let keyWidth:CGFloat = min(UIScreen.main.bounds.width / 12,10)
let letterWidth:CGFloat = UIScreen.main.bounds.height / 50
...
struct HeaderView: View {
    ...
    var body: some View {
        ...
        VStack {
            ...
            Text(String(value)).font(.subheadline)
...
struct ContentView: View {
    ...
    @State var fontBig:Font = .headline
    @State var message = ""
    ...
    var body: some View {
            ...
            ForEach(grid.rows, id: \.id) { row in
                ...
                HStack(spacing: 10){
                    ...
                    ForEach(row.entries, id: \.id) { entry in
                        ...
                        Button(
                            action: { selectEntry(id: entry.id) },
                            label: {
                                Text(entry.value)
        ...
        if message == "" {
                ...
                ForEach(keyboard.rows, id: \.id) { row in
                    ...
                    HStack(spacing: 10){
                        ...
                        ForEach(row.keys, id: \.id) { key in
                            ...
                            Button(
                                action: { keyPressed(key: key.value) },
            ...
            .onExitCommand(perform: game.cursorX == 0 ? nil : {
                keyPressed(key:"⌫")
            })
        }
        //  MESSAGE BUTTON
    ...
    func updateGrid(row:Int, index:Int,value:String,status:LetterStatus) {
        ...
        case .STS_PENDING:
            ...
            grid.rows[row].entries[index].background = Color("LightGray")
            ...
            for (x,key) in row.keys.enumerated() {
                ...
                if key.value == value {
                    ...
                    case .STS_PENDING:
                        ...
                        keyboard.rows[y].keys[x].background = Color("LightGray")
